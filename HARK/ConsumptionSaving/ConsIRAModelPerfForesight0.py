...
class IRAPerfForesightConsumerType(HARKobject):
    ...
    def solve(self):
        ...
        solution = [ConsIRAPFterminal0(CRRA=self.CRRA,PenIRA=PenT,
                                       FixedCost=self.FixedCost)]
        
        # Add rest of solutions
        for i in reversed(range(self.T_cycle-1)):
            if i >= self.T_ira - 1:
                solution.append(ConsIRAPFnoPen0(
...
def main():
    ...
    IRAPF.solve()
    IRAPF.simulate(w0)
    IRAPF.graphSim(saveFig=1,savePath='IRA_Results0',graphLab='7p')
