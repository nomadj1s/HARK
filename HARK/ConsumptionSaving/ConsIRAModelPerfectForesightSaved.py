...
class NoPenSolution(HARKobject):
    ...
    def __call__(self,m,n):
        ...
        return corner_1*interior_1 # This is not complete, needs to be fixed
...
class ConsIRA5Period3(HARKobject):
    ...
    def __call__(self,m,n):
        ...
        if d['inter'] < -n: # lower bound on withdrawal is binding
            ...
            d['liq'] = -n
            a['liq'] = 0.0
            v['liq'] = u(c['liq']) + b*self.ConsIRA5Period4(y4,0)['vFunc']
            vPm['liq'] = uP(c['liq'])
            vPn['liq'] = uP(c['liq'])
        ...
        if d['inter'] >= -n and d['inter'] <= dMax: # neither bound binds
            ...
            a['inter'] = 0.0
            v['inter'] = u(c['inter']) +\
                         b*self.ConsIRA5Period4(y4,r*(n+d['inter']))['vFunc']
...
class ConsIRAPFnoPen(HARKobject):
    ...
    def __call__(self,m,n):
        ...
        vPn[...,2][cap] = r*b*self.ConsIRAnext(yN[cap],
        ...
        vPn[...,3][cap_save] = r*b*self.ConsIRAnext(yN[cap_save]
                                                    +ra*a[...,3][cap_save],
                                                    r*(n[cap_save]+dMax)
                                                    )['vPnFunc']
...
class ConsIRA5Period2(HARKobject):
    ...
    def wFOC(self,d,m,n):
        '''
        Evaluate expression for d, derived from the FOC for d in period 2 at
    ...
    def __call__(self,m,n):
        ...
        solution['vPmFunc'] : float
            Marginal value function wrt m in period 2.
        solution['vPnFunc'] : float
            Marginal value function wrt n in period 2.
        '''
        ...
        if d['with'] < -n: # lower bound on withdrawal is binding
            ...
            d['liq'] = -n
            a['liq'] = 0.0
            v['liq'] = u(c['liq']) + b*self.ConsIRA5Period3(y3,0)['vFunc']
            vPm['liq'] = uP(c['liq'])
        ...
        if d['with'] >= -n and d['with'] <= 0.0: # neither bound binds
            ...
            a['with'] = 0.0
            v['with'] = u(c['with']) +\
                        b*self.ConsIRA5Period3(y3,r*(n+d['with']))['vFunc']
            vPm['with'] = uP(c['with'])
        ...
        if d['dep'] >= 0.0 and d['dep'] <= dMax:
            ...
            a['dep'] = 0.0
            v['dep'] = u(c['dep']) +\
                       b*self.ConsIRA5Period3(y3,r*(n+d['dep']))['vFunc']
            vPm['dep'] = uP(c['dep'])
            vPn['dep'] = uP(c['dep'])
...
class ConsIRA5Period1(HARKobject):
    '''
    Closed form solution for 5-period IRA consumer with perfect foresight.
    Period 4 is the last period (the first period is 0).
    '''
    ...
    def __init__(self,NextIncome,DiscFac,CRRA,Rsave,Rira,PenIRA,MaxIRA,
        ...
        self.MaxIRA         = MaxIRA
        self.y2             = NextIncome
        self.ConsIRA5Period2 = deepcopy(ConsIRA5Period2)
        self.output         = output
    ...
    def wFOC(self,d,m,n):
        ...
        g = self.CRRA
        y2 = self.y2
        vPn = self.ConsIRA5Period2(y2,r*(n + d))['vPnFunc']
    ...
    def aKinkFOC(self,a,m,n):
        '''
        Evaluate expression for a, derived from the FOC for a in period 1 when
        illiquid savings are at a kink. Not a closed form solution, since it 
        ...
        g = self.CRRA
        y2 = self.y2
        vPm = self.ConsIRA5Period2(y2 + ra*a,r*n)['vPmFunc']
    ...
    def aFOC(self,a,m,n):
        ...
        y2 = self.y2
        dMax = self.MaxIRA
        vPm = self.ConsIRA5Period2(y2 + ra*a,r*(n+dMax))['vPmFunc']
...
class ConsIRAPFpen(HARKobject):
    ...
    def aFOC(self,a,m,n,yN):
        '''
        Evaluate expression for a, derived from the FOC for a when illiquid 
        savings are capped. Not a closed form solution, since it also 
        depends on a and the value function of the next period.
    ...
    def __call__(self,m,n):
        ...
        vPn[...,3][kink_save] = r*b*self.ConsIRAnext(yN[kink_save] 
                                                     + ra*a[...,3][kink_save],
