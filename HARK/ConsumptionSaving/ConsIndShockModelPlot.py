...
def main():
    ...
    if do_simulation:
        ...
        KinkedExample.track_vars = ['t_age','aNrmNow','mNrmNow','cNrmNow','pLvlNow']
        KinkedExample.initializeSim()
        KinkedExample.simulate()
    
    def sim_avg(hist_array,age_array,p_array,t):
        inflated = hist_array[age_array == t]*p_array[age_array == t]
        averaged = np.average(inflated)
        return averaged

    plot_list = [KinkedExample.mNrmNow_hist,
                 KinkedExample.cNrmNow_hist,
                 KinkedExample.aNrmNow_hist]

    ages = KinkedExample.t_age_hist
    pLvls = KinkedExample.pLvlNow_hist
    period_T = np.amax(ages)

    m,c,a = [[sim_avg(j,ages,pLvls,i) for i in range(1,period_T+1)]
             for j in plot_list]    
    ...
    plt.xlabel("age")
    plt.ylabel("level")
    plt.title("Kinked Consumer")
    plt.legend()
    plt.grid()
    plt.savefig('IRA_Results/KinkedTimeSeries.png')
    plt.show()
